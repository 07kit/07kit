...
public class ComponentResizer extends MouseAdapter {
    ...
    protected void changeBounds(Component source, int direction, Rectangle bounds, Point pressed, Point current) {
        ...
        if (WEST == (direction & WEST)) {
            ...
            // "hack" (not really) to prevent it from moving the client when it hits the minimum size
            if (width < source.getMinimumSize().width) {
                return;
            }
        }
    ...
    private Dimension getBoundingSize(Component source) {
        if (source instanceof Window) {
            ...
            for (final GraphicsDevice gd : env.getScreenDevices()) {
                d.width += gd.getDefaultConfiguration().getBounds().width + 200;
                d.height += gd.getDefaultConfiguration().getBounds().height + 200;
            }
            return d;
        } else {
